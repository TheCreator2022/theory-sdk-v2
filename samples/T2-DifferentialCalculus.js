...
var init = () => {
    ...
    theory.createAutoBuyerUpgrade(2, currency, 1e3.5);
...
var getPrimaryEquation = () => {
    ...
    if (q1Exp.level == 1) result += "^{1.5}";
    if (q1Exp.level == 2) result += "^{3}";
    if (q1Exp.level == 3) result += "^{3.5}";
    result += "r_1";
    if (r1Exp.level == 1) result += "^{1.5}";
    if (r1Exp.level == 2) result += "^{3}";
    if (r1Exp.level == 3) result += "^{3.5}";
...
var getPublicationMultiplierFormula = (symbol) => "\\frac{{" + symbol + "}^{3}}{1}";
